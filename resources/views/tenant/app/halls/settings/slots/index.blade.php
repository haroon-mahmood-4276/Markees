...
@section('seo-breadcrumb')
    {{ Breadcrumbs::view('breadcrumbs::json-ld', 'tenant.halls.slots.index', $hall->id) }}
@endsection
...
@section('breadcrumbs')
    <div class="d-flex justify-content-start align-items-center mb-3">
        ...
        {{ Breadcrumbs::render('tenant.halls.slots.index', $hall->id) }}
    </div>
@endsection
...
@section('content')
    <div class="nav-align-top">
        ...
        @include('tenant.app.halls.settings.partials.tabs')

        <div class="tab-content">
            <div class="tab-pane fade show active" id="navs-pills-slots" role="tabpanel">
                <form action="{{ route('tenant.halls.slots.destroy', [$hall]) }}" id="hall-slots-table-form" method="get">
                    {{ $dataTable->table() }}
                </form>
            </div>
        </div>
...
@section('custom-js')
    ...
    <script>
        ...
        function addNew() {
            location.href = "{{ route('tenant.halls.slots.create', [$hall]) }}";
        }
    </script>
@endsection
